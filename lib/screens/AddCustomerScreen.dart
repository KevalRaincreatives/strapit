...
import 'package:strapit/models/SignUpModel.dart';
...
class _AddCustomerScreenState extends State<AddCustomerScreen> {
  ...
  Future<String?> getRegister() async {
    ...
    try {
      ...
      final msg = jsonEncode({"email": emailCont.text, "password": passwordCont.text, "confirm_password": passwordCont.text,
      "name": nameCont.text, "phone": phoneCont.text, "country": _selectedDialogCountry.name});
      ...
      try {
        response=await client.post(
            Uri.parse(
                'https://strapit.rcstaging.co.in/strapit/public/api/register'),
            headers: headers,
            body: msg);
      } finally {
        client.close();
      ...
      if (response.statusCode == 200) {
        //
        signUpModel = new SignUpModel.fromJson(jsonResponse);
